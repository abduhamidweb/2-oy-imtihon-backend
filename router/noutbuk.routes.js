...
noutbuk.get("/", (req, res) => {
    ...
    .post("/", (req, res) => {
        let obServerData = {
            id: obServer.length ? obServer[obServer.length - 1].id + 1 : 1,
            name: req.method,
            event: "Noutbuklarga Post bo'ldi."
        }
        obServer.push(obServerData)
        fs.writeFileSync(
            process.cwd() + "/db/observer.json",
    ...
    .put("/:id", (req, res) => {
        ...
        let obServerData = {
            id: obServer.length ? obServer[obServer.length - 1].id + 1 : 1,
            name: req.method,
            event: "Noutbukdagi " + id + " ga o'zgartirish kiritilidi."
        }
        obServer.push(obServerData)
        fs.writeFileSync(
            process.cwd() + "/db/observer.json",
        ...
        try {
            ...
            if (!product) throw new Error("Not found " + id + "-user");
            if (product.category == CATEGORY) {
    ...
    .delete("/:id", (req, res) => {
        let {
            id
                ...
                let obServerData = {
                    id: obServer.length ? obServer[obServer.length - 1].id + 1 : 1,
                    name: req.method,
                    event: "Noutbukdagi " + id + " o'chirildi."
                }
                obServer.push(obServerData)
                fs.writeFileSync(
                    process.cwd() + "/db/observer.json",
        ...
        if (find.category == CATEGORY) {
            ...
            res.send({
                ...
                accsept: `BU ${CATEGORY} ni siz delete qila olmaysiz..`
            })
        }
    })
